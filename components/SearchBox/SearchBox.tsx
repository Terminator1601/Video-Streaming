...
const SearchBox: React.FC<Props> = ({ onSearch }) => {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => { // Specify type React.ChangeEvent<HTMLInputElement> for e
    setQuery(e.target.value);
  };

  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => { // Specify type React.FormEvent<HTMLFormElement> for e
    e.preventDefault();
    onSearch(query);
  };
